...
const searchForUser = (
  userList: { nome: string; idade: number; status: boolean }[],
  userName: string
) => {
  let userFound = [];
  ...
  for (const user of userList) {
    if (user.nome.toLowerCase().includes(lowerCaseUserName)) {
      userFound.push(user);
    }
  }
...
console.log(searchForUser(users, "jo"));
...
// console.log(filtrarUsuarios(lista, "guido"));
